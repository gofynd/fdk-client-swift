...
```swift
catalog.getProductPriceBySlug(slug: slug, size: size, storeId: storeId, pincode: pincode, depth: depth, strategy: strategy) { (response, error) in
    // Use response
}
```
...
```swift
catalog.getProductSellersBySlug(slug: slug, size: size, pincode: pincode, strategy: strategy, pageNo: pageNo, pageSize: pageSize, depth: depth) { (response, error) in
    // Use response
}
```
