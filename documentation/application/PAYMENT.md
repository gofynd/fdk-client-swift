...
```swift
payment.outstandingOrderDetails(merchantUserId: merchantUserId) { (response, error) in
    // Use response
}
```
...
```swift
payment.paidOrderDetails(merchantUserId: merchantUserId) { (response, error) in
    // Use response
}
```
