...
```swift
applicationClient.order.getInvoiceByShipmentId(shipmentId: shipmentId, documentType: documentType) { (response, error) in
    // Use response
}
```
...
```swift
applicationClient.order.getProducts(status: status, pageNo: pageNo, pageSize: pageSize, fromDate: fromDate, toDate: toDate, searchValue: searchValue) { (response, error) in
    // Use response
}
```
...
```swift
applicationClient.order.updateShipmentStatus1(shipmentId: shipmentId, body: body) { (response, error) in
    // Use response
}
```
...
| body | UpdateShipmentStatusRequest1 | yes | Request body |
...
---
 ...
 | Properties | Type | Nullable | Description |
 | ---------- | ---- | -------- | ----------- |
 | filters | [[ProductsDataUpdatesFilters1](#ProductsDataUpdatesFilters1)]? |  yes  | Filter for the Product/Bag |
 | data | [String: Any]? |  yes  |  |
...
---
 ...
 | products | [[Products1](#Products1)]? |  yes  | Product/Bag to be updated |
