...
```swift
client.order.getProducts(status: status, pageNo: pageNo, pageSize: pageSize, fromDate: fromDate, toDate: toDate, customMeta: customMeta, searchType: searchType, searchValue: searchValue) { (response, error) in
    // Use response
}
```
