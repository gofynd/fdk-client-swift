...
```swift
cart.getCart(id: id, i: i, b: b, assignCardId: assignCardId, buyNow: buyNow) { (response, error) in
    // Use response
}
```
...
```swift
cart.getAddresses(cartId: cartId, buyNow: buyNow, mobileNo: mobileNo, checkoutMode: checkoutMode, tags: tags, isDefault: isDefault, userId: userId) { (response, error) in
    // Use response
}
```
...
```swift
cart.removeAddress(id: id, userId: userId) { (response, error) in
    // Use response
}
```
...
```swift
cart.selectAddress(cartId: cartId, buyNow: buyNow, i: i, b: b, userId: userId, body: body) { (response, error) in
    // Use response
}
```
