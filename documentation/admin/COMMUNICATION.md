...
```swift
communication.sendSellerCommunicationSynchronously(body: body) { (response, error) in
    // Use response
}
```
...
```swift
communication.sendSellerCommunicationAsynchronously(body: body) { (response, error) in
    // Use response
}
```
