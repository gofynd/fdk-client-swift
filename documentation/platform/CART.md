...
```swift
client.application("<APPLICATION_ID>").cart.getCartList(fromDate: fromDate, toDate: toDate, sortOn: sortOn) { (response, error) in
    // Use response
}
```
...
```swift
client.application("<APPLICATION_ID>").cart.deleteCart(id: id) { (response, error) in
    // Use response
}
```
