...
```swift
client.application("<APPLICATION_ID>").rewards.getOfferByName(name: name) { (response, error) in
    // Use response
}
```
...
```swift
client.application("<APPLICATION_ID>").rewards.getUserDetails(userId: userId) { (response, error) in
    // Use response
}
```
