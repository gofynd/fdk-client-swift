...
```swift
client.application("<APPLICATION_ID>").payment.getPosPaymentModeRoutes() { (response, error) in
    // Use response
}
```
...
```swift
client.application("<APPLICATION_ID>").payment.getPosPaymentModeRoutes(body: body) { (response, error) in
    // Use response
}
```
