...
```swift
client.catalog.getSizeGuides(active: active, q: q, tag: tag, pageNo: pageNo, pageSize: pageSize, brandId: brandId) { (response, error) in
    // Use response
}
```
...
```swift
client.catalog.listDepartmentsData(pageNo: pageNo, pageSize: pageSize, name: name, search: search, isActive: isActive, itemType: itemType) { (response, error) in
    // Use response
}
```
...
```swift
client.catalog.listProductTemplateExportDetails(status: status, fromDate: fromDate, toDate: toDate, q: q) { (response, error) in
    // Use response
}
```
...
```swift
client.catalog.patchProduct(itemId: itemId, body: body) { (response, error) in
    // Use response
}
```
...
```swift
client.catalog.getInventoryExport(status: status, fromDate: fromDate, toDate: toDate, q: q) { (response, error) in
    // Use response
}
```
...
---
 ...
 | Properties | Type | Nullable | Description |
 | ---------- | ---- | -------- | ----------- |
 | items | [[GetMultiSizeGuide](#GetMultiSizeGuide)]? |  yes  |  |
 | page | [String: Any]? |  yes  |  |
...
---
 ...
 | ---------- | ---- | -------- | ----------- |
 | tag | String? |  yes  |  |
 | attributesSchema | [[String: Any]]? |  yes  |  |
 | isActive | Bool? |  yes  |  |
 | name | String? |  yes  |  |
 | createdOn | String? |  yes  |  |
