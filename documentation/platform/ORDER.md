...
```swift
platformClient.order.getRoleBaseStateTransition(orderingChannel: orderingChannel, status: status) { (response, error) in
    // Use response
}
```
