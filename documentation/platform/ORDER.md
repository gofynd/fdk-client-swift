...
```swift
platformClient.order.getShipmentById(channelShipmentId: channelShipmentId, shipmentId: shipmentId, fetchActiveShipment: fetchActiveShipment, allowInactive: allowInactive) { (response, error) in
    // Use response
}
```
...
| allowInactive | Bool? | no | Flag indicating whether inactive shipments are allowed |  
