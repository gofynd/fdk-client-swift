...
```swift
client.order.getPlatformShipmentReasons(shipmentId: shipmentId, bagId: bagId, state: state) { (response, error) in
    // Use response
}
```
...
```swift
client.order.manualAssignDPToShipment(body: body) { (response, error) in
    // Use response
}
```
