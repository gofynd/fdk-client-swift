...
public extension PlatformClient {
    class DocumentEngine {
         ...
         **/
        public func generatePOSReceipts(
            body: GeneratePOSReceipts,
            onResponse: @escaping (_ response: SuccessResponseGeneratePOSReceipts?, _ error: FDKError?) -> Void
        ) {
            PlatformAPIClient.execute(
                config: config,
                method: "post",
                url: "/service/platform/document/v2.0/company/\(companyId)/pos/generate-receipt",
                query: nil,
                body: body.dictionary,
                headers: [],
                responseType: "application/json",
                onResponse: { responseData, error, responseCode in
                    if let _ = error, let data = responseData {
                        ...
                        let response = Utility.decode(SuccessResponseGeneratePOSReceipts.self, from: data)
