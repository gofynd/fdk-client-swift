...
extension PlatformClient {
    ...
    public class Order {        
        ...
        * Description: It is used for updating the shipment in the following scenarios: Full Confirmation, Partial Confirmation, Change the status of a shipment, Full Cancellation, Partial Cancellation, Assign the shipment to DP. Click <a href='/commerce/miscellaneous/updateShipmentStatus'>here</a> to get the use case details.
        **/
...
The ESM config stores order processing configuration. Each document in the ESM config collection of  Order Management System - OMS's database is a JSON object representing the configuration of a specific application ID. This includes filters, hooks, flags set on different state-transitions.  This configuration is picked and accordingly features are enabled. 
        ...
        * Description: Retrieve all packages associated with a specific shipment. This endpoint supports  both single-piece and multi-piece shipments.
        **/
        public func getShipmentPackages(
            shipmentId: String,
            
            headers: [(key: String, value: String)]? = nil,
            onResponse: @escaping (_ response: PackagesResponseSchema?, _ error: FDKError?) -> Void
        ) {
            ...
            PlatformAPIClient.execute(
                config: config,
                method: "GET",
                url: "/service/platform/order-manage/v1.0/company/\(companyId)/shipment/\(shipmentId)/packages",
                query: nil,
                body: nil,
                headers: xHeaders,
                responseType: "application/json",
                onResponse: { (responseData, error, responseCode) in
                    if let _ = error, let data = responseData {
                        ...
                        let response = Utility.decode(PackagesResponseSchema.self, from: data)
        ...
        **/
        public func createShipmentPackages(
            shipmentId: String,
            body: PackagesSchema,
            headers: [(key: String, value: String)]? = nil,
            onResponse: @escaping (_ response: BaseResponseSchema?, _ error: FDKError?) -> Void
        ...
        * Description: Update existing packages for a shipment. This operation replaces all existing  packages with the provided package list. The system validates courier partner  availability and performs bag breaking as per number of packages. Any packages  without IDs will have new unique IDs generated.
        **/
        public func updateShipmentPackages(
            shipmentId: String,
            body: PackagesSchema,
            headers: [(key: String, value: String)]? = nil,
            onResponse: @escaping (_ response: BaseResponseSchema?, _ error: FDKError?) -> Void
        ...
        **/
        public func requestCourierPartnerForShipment(
            shipmentId: String,
            body: ShipmentCourierPartnerRequestSchema,
            headers: [(key: String, value: String)]? = nil,
            onResponse: @escaping (_ response: CourierPartnerResponseSchema?, _ error: FDKError?) -> Void
        ) {
            ...
            PlatformAPIClient.execute(
                config: config,
                method: "POST",
                url: "/service/platform/order-manage/v1.0/company/\(companyId)/shipment/\(shipmentId)/courier-partner/request",
                query: nil,
                body: body.dictionary,
                headers: xHeaders,
                responseType: "application/json",
                onResponse: { (responseData, error, responseCode) in
                    if let _ = error, let data = responseData {
                        ...
                        let response = Utility.decode(CourierPartnerResponseSchema.self, from: data)
        ...
        **/
        public func saveCourierPartnerPreferenceForShipment(
            shipmentId: String,
            body: ShipmentCourierPartnerPreference,
            headers: [(key: String, value: String)]? = nil,
            onResponse: @escaping (_ response: CourierPartnerResponseSchema?, _ error: FDKError?) -> Void
        ) {
            ...
            PlatformAPIClient.execute(
                config: config,
                method: "POST",
                url: "/service/platform/order-manage/v1.0/company/\(companyId)/shipment/\(shipmentId)/courier-partner/preference",
                query: nil,
                body: body.dictionary,
                headers: xHeaders,
                responseType: "application/json",
