...
public extension PlatformClient {
    class Order {
         ...
         * Description:
         **/
        public func getShipmentTracking(
            shipmentId: String,

            onResponse: @escaping (_ response: ShipmentTrackingResponse?, _ error: FDKError?) -> Void
        ) {
            var xQuery: [String: Any] = [:]
            ...
            PlatformAPIClient.execute(
                config: config,
                method: "get",
                url: "/service/platform/dp/v1.0/company/\(companyId)/tracking",
                query: xQuery,
                body: nil,
                headers: [],
                responseType: "application/json",
                onResponse: { responseData, error, responseCode in
                    if let _ = error, let data = responseData {
                        ...
                        let response = Utility.decode(ShipmentTrackingResponse.self, from: data)
