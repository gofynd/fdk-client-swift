...
public extension PlatformClient {
    class DocumentEngine {
         ...
         **/
        public func getBulkShipmentStatus(
            batchId: String,

            onResponse: @escaping (_ response: SuccessResponseBulkStatus?, _ error: FDKError?) -> Void
        ) {
            var xQuery: [String: Any] = [:]
            ...
            PlatformAPIClient.execute(
                config: config,
                method: "get",
                url: "/service/platform/document/v2.0/company/\(companyId)/bulk-shipment-status",
                query: xQuery,
                body: nil,
                headers: [],
                responseType: "application/json",
                onResponse: { responseData, error, responseCode in
                    if let _ = error, let data = responseData {
                        ...
                        let response = Utility.decode(SuccessResponseBulkStatus.self, from: data)
