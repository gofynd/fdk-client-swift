...
public extension PlatformClient {
    class OrderManage {
         ...
         * Description:
         **/
        public func statusInternalUpdate(
         ...
         **/
        public func manualStoreReassignment(
            body: ManualStoreReassign,
            onResponse: @escaping (_ response: ManualStoreReassignResponse?, _ error: FDKError?) -> Void
        ) {
            PlatformAPIClient.execute(
                config: config,
                method: "post",
                url: "/service/platform/order-manage/v1.0/company/\(companyId)/store/manual-reassign",
                query: nil,
                body: body.dictionary,
                headers: [],
                responseType: "application/json",
                onResponse: { responseData, error, responseCode in
                    if let _ = error, let data = responseData {
                        ...
                        let response = Utility.decode(ManualStoreReassignResponse.self, from: data)
