...
public extension ApplicationClient.Theme.ActionPage {
    ...
    static func convertURLToAction(urlString: String) -> ApplicationClient.Theme.ActionPage? {
        return ApplicationClient.Theme.ActionPage(urlString: urlString)
    }
    ...
    convenience init?(urlString: String) {
        ...
        guard let urlComponents = urlComp else { return nil }
        guard let type = ApplicationClient.Theme.PageType(path: urlComponents.percentEncodedPath) else { return nil }
        for item in urlComponents.queryItems ?? [] {
            if let queryValue = item.value {
                if var list = query[item.name] {
                    list.append(queryValue)
